...
# --- 戦略的30地点定義 (JMAコード & 緯度経度) ---
# JMAコードは「府県予報区コード」を使用
TARGET_AREAS = {
    # --- 北海道・東北 ---
    ...
    # --- 東京・関東 (気象庁コードは東京130000で共通、緯度経度と特性で差別化) ---
    ...
    "chiba_maihama": { "name": "千葉 舞浜(ディズニー)", "jma_code": "120010", "lat": 35.6329, "lon": 139.8804, "feature": "ディズニーリゾート。イベントと天候への依存度極大。" },
    "kanagawa_yokohama": { "name": "神奈川 横浜", "jma_code": "140010", "lat": 35.4437, "lon": 139.6380, "feature": "みなとみらい観光とビジネスが融合。中華街。" },
    # --- 中部 ---
    "aichi_nagoya": { "name": "愛知 名古屋", "jma_code": "230010", "lat": 35.1815, "lon": 136.9066, "feature": "トヨタ系ビジネスと独自の飲食文化。車社会。" },
    # --- 関西 ---
    ...
    "kyoto_shijo": { "name": "京都 四条河原町", "jma_code": "260010", "lat": 35.0037, "lon": 135.7706, "feature": "世界最強の観光都市。インバウンド需要が桁違い。" },
    "hyogo_kobe": { "name": "兵庫 神戸(三宮)", "jma_code": "280010", "lat": 34.6946, "lon": 135.1956, "feature": "オシャレな港町。観光とビジネス。" },
    # --- 中国・九州・沖縄 ---
    "hiroshima": { "name": "広島", "jma_code": "340010", "lat": 34.3853, "lon": 132.4553, "feature": "平和公園・宮島。欧米系インバウンド多い。" },
    "fukuoka": { "name": "福岡 博多・中洲", "jma_code": "400010", "lat": 33.5902, "lon": 130.4017, "feature": "アジアの玄関口。屋台文化など夜の需要が強い。" },
    "okinawa_naha": { "name": "沖縄 那覇", "jma_code": "471010", "lat": 26.2124, "lon": 127.6809, "feature": "国際通り。観光客メイン。台風等の天候影響大。" },
}
...
def get_jma_forecast(area_code):
    ...
    try:
        with urllib.request.urlopen(url, timeout=10) as res:
            ...
            temp_series = data[0]["timeSeries"][2] # 気温
...
def get_model():
    ...
    try:
        return genai.GenerativeModel(target_model, tools=tools, generation_config=generation_config)
    except:
        return genai.GenerativeModel('models/gemini-1.5-flash', tools=tools, generation_config=generation_config)
...
def get_ai_advice(area_key, area_data, target_date, jma_data):
    ...
    date_display = target_date.strftime('%m月%d日') # 表示用
    ...
    # プロンプト (検索指示含む)
    prompt = f"""
    あなたは世界屈指の戦略経営コンサルタントです。Google検索を活用し、ファクトに基づいた戦略を提案してください。
    ...
    【重要指令】
    1. **Google検索を実行せよ:** "{area_data['name']} イベント {date_str}", "{area_data['name']} クルーズ船 {date_str[:7]}", "{area_data['name']} 混雑予想" などを検索し、具体的なイベント名や社会的動向（インバウンド、受験、セール等）を特定せよ。
    2. **挨拶・自己紹介は一切禁止:** いきなり分析結果から書き始めよ。
    3. **「レポート」項目の構成:** - タイトル: 「{date_display}のレポート」
       - 結論: 1行でズバリ（例：〇〇のためタクシー需要はSランクです）
       - 要因: 検索で得た事実を箇条書き
       - 戦略: 各職種への具体的なアクションプラン
    4. **口調:** 「〜しましょう」ではなく「〜が有効です」「〜を推奨します」という提案口調で統一せよ。
    5. **整合性:** 雨確率が低いなら「雨」と言うな。気温が低いなら「防寒」を考慮せよ。
    6. **出力:** 以下のJSON形式のみを出力せよ。Markdownタグは不要。
...
def get_simple_forecast(target_date):
    ...
    # 簡易的な平年値ロジック (本来はOpen-Meteoなどで補完すべきだが今回は簡易版)
    rank = "C"
...
if __name__ == "__main__":
    ...
    print(f"🦅 Eagle Eye 30地点・検索強化版 起動: {today.strftime('%Y/%m/%d')}", flush=True)
